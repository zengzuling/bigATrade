...
```powershell
$env:BIGATRADE_DB_HOST="<db-host>"
$env:BIGATRADE_DB_PORT="<db-port>"
$env:BIGATRADE_DB_USER="<db-user>"
$env:BIGATRADE_DB_PASSWORD="<db-password>"

python -m bigatrade recommend --date 2026-06-05 --scan-limit all --top 2000 --price-buckets "0-10:2,10-20:2,20-50:1" --prefilter-per-bucket "0-10:120,10-20:120,20-50:80" --hotspot-db-host $env:BIGATRADE_DB_HOST --hotspot-db-port $env:BIGATRADE_DB_PORT --hotspot-db-user $env:BIGATRADE_DB_USER --hotspot-db-password $env:BIGATRADE_DB_PASSWORD --hotspot-top-industry 20 --hotspot-top-concept 20 --output outputs/recommend_2026-06-05_hotspot.csv
```
...
```powershell
python -m bigatrade daily-run --date today --scan-limit all --top 30 --price-buckets "0-10:2,10-20:2,20-50:1" --prefilter-per-bucket "0-10:120,10-20:120,20-50:80" --db-host $env:BIGATRADE_DB_HOST --db-port $env:BIGATRADE_DB_PORT --db-user $env:BIGATRADE_DB_USER --db-password $env:BIGATRADE_DB_PASSWORD
```
...
```powershell
python -m bigatrade settle-results --date today --db-host $env:BIGATRADE_DB_HOST --db-port $env:BIGATRADE_DB_PORT --db-user $env:BIGATRADE_DB_USER --db-password $env:BIGATRADE_DB_PASSWORD
python -m bigatrade five-day-summary --date today --db-host $env:BIGATRADE_DB_HOST --db-port $env:BIGATRADE_DB_PORT --db-user $env:BIGATRADE_DB_USER --db-password $env:BIGATRADE_DB_PASSWORD
python -m bigatrade review --date today --db-host $env:BIGATRADE_DB_HOST --db-port $env:BIGATRADE_DB_PORT --db-user $env:BIGATRADE_DB_USER --db-password $env:BIGATRADE_DB_PASSWORD
```
